...
export const EXPECTED_STAGE_A = Object.freeze({completed: 60, documents: 103, sources: 535});
...
const DIAGRAM_NODES = Object.freeze([
  'actor-canvas', 'shared-state-boundary', 'actor-runtime-boundary', 'external-authority-boundary', 'legend-band',
...
test('governs STY-08 sources, reciprocal relations, and Stage A projection', async () => {
  ...
  const status = JSON.parse(readFileSync('src/generated/project-status.json', 'utf8')); assert.deepEqual({completed: status.completed_topics, documents: status.content_documents, sources: status.governed_sources}, EXPECTED_STAGE_A);
