...
function assertCombinedProjection(statusValue = status, manifestValue = manifest, indexesValue = indexes, ledgerValue = publicLedger) {
  assert.deepEqual(
    ...
    {completed: 63, documents: 107, sources: 560},
  );
  assert.equal(ledgerValue.sources.length, 560);
  ...
  assert.deepEqual([topics.get('STY-11')?.published, topics.get('STY-11')?.status, styles.get('STY-11')?.published], [true, {scope: 'backlog-projection', value: 'pending', source: 'docs/content-backlog.md'}, true]);
