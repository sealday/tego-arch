...
const reviewFile = new URL('../docs/reviews/g006-batch2.md', import.meta.url);
const topics = new Map([
  ['QA-04', '/quality-attributes/qa-04'],
  ['QA-06', '/quality-attributes/qa-06'],
  ['QA-07', '/quality-attributes/qa-07'],
]);
...
async function readRequiredReview() {
  try {
    ...
    if (error.code === 'ENOENT') {
      assert.fail(`Missing G006 Batch 2 review evidence: ${reviewFile.pathname}`);
    }
    throw error;
  }
}
...
test('requires a non-empty G006 Batch 2 review record', async () => {
...
test('records visible editorial fact copyright and render PASS per article', async () => {
  ...
  for (const id of topics.keys()) {
    ...
    assert.match(
      section,
      /\bdesktop\s+`?1440x1000`?/iu,
      `${id} desktop evidence`,
    );
    assert.match(
      section,
      /\bmobile\s+`?390x844`?/iu,
      `${id} mobile evidence`,
    );
    assert.match(
      section,
      /deterministic representation[^。\n]{0,80}\bPASS\b|\bPASS\b[^。\n]{0,80}deterministic representation/iu,
...
test('records actual reciprocal clicks plus license and anti-overclaim findings', async () => {
  const review = visibleReview(await readRequiredReview());

  assert.match(
    review,
    /reciprocal clicks?[^\n]*\bPASS\b|\bPASS\b[^\n]*reciprocal clicks?/iu,
    'Review evidence must record reciprocal clicks PASS',
  );
  assert.match(
    review,
    /license findings?[^\n]*\bPASS\b|\bPASS\b[^\n]*license findings?/iu,
    'Review evidence must record license findings PASS',
  );
  assert.match(
    review,
    /anti-overclaim findings?[^\n]*\bPASS\b|\bPASS\b[^\n]*anti-overclaim findings?/iu,
    'Review evidence must record anti-overclaim findings PASS',
  );
});
