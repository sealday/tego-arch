...
const drawioPath = 'diagrams/multi-agent-research-system.drawio';
const svgPath = 'static/img/diagrams/multi-agent-research-system.svg';
const nodeTitles = [
...
const relationLabels = [
  ...
  'citations verified',
  'citation mismatch',
  'bounded retrieval round',
  'low authority / conflict',
  'publish / abstain',
  'budget-stop',
...
function cellInventory(drawioRoot) {
  ...
  for (const cell of cells) {
    ...
    if (relationLabels.includes(value)) relationEdges.set(value, lastEdge);
    if (cell.attributes.get('vertex') === '1' && !value) lastShape = cell.attributes.get('id');
  }
  return {byValue, nodeShapes, relationEdges};
}
...
function validateGeometry(drawio, svg) {
  ...
  assert.deepEqual([...inventory.nodeShapes.keys()].sort(), [...nodeTitles].sort(), 'all ten node mappings');
  assert.deepEqual([...inventory.relationEdges.keys()].sort(), [...relationLabels].sort(), 'all fourteen relation mappings');
...
test('multi-agent research diagram enforces measured geometry for all nodes and relationships', () => {
  const drawio = readFileSync(drawioPath, 'utf8');
  const svg = readFileSync(svgPath, 'utf8');
  const audit = validateGeometry(drawio, svg);
  if (process.env.REPORT_MULTI_AGENT_RESEARCH_GEOMETRY === '1') {
...
test('multi-agent research geometry rejects node, arrow, foreign-connector, and opaque-background mutants', () => {
  ...
  assert.ok(validateGeometry(drawio, opaque).failures.includes('bounded retrieval round: opaque label background'));
});
