...
test('keeps every source and evidence field in the complete sorted model', async () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      ['paper', 19],
      ['official-docs', 176],
      ['official-repository', 32],
      ['source-code', 157],
      ['engineering-blog', 11],
      ['incident-report', 0],
      ['vendor-reference-architecture', 19],
      ['textbook', 7],
      ['independent-blog', 11],
      ['community-index', 7],
      ['original-illustration', 27],
    ],
  );
  assert.equal(new Set(cards.map(({id}) => id)).size, 481);
