...
test('preserves immutable Batch 12 history under current DDD-01 complete projection', () => {
  ...
  assert.deepEqual({published: next?.published, status: next?.status?.value}, {published: true, status: 'complete'});
  assert.ok(documents.some(({metadata}) => metadata.topic_id === NEXT_TOPIC), 'STY-13 content is published');
  assert.equal(documents.some(({metadata}) => metadata.topic_id === 'STY-14'), true, 'current STY-14 content is published');
  assert.equal(documents.flatMap(extractInternalLinks).includes('/styles/sty-14'), true, 'current STY-14 is actionable');
