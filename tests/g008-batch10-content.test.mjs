...
test('locks the generated MOD-13 Stage B projection', () => {
  assert.deepEqual({
    ...
    next_pending_style: topicManifest.topics.find(({type, status}) => type === 'style' && status.value === 'pending')?.id,
  }, {
    completed_topics: 85,
  ...
  const staleNextTopic = backlog.replace('下一项为 STY-11', '下一项为 STY-10');
