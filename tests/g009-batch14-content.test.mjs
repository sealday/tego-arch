...
export const REMOTE_CITATION_NOTES = Object.freeze([
  ...
  ['src-oracle-coherence-partitioned-cache', 'Introduction to Coherence, Oracle, Coherence 12.2.1.4', 'Original Chinese factual summary limited to documented Coherence partitioned-data and rebalancing mechanisms as a narrow comparison; no backup/storage claim and no source prose, structure or diagrams copied.'],
  ['src-oracle-coherence-backing-maps', 'Implementing Storage and Backing Maps, Oracle, Coherence 12.2.1', 'Original Chinese factual summary independently limited to documented Coherence backup/storage, backing-map and persistent-store mechanisms as a narrow recovery comparison; no source prose, structure or diagrams copied.'],
...
export const ORACLE_BOUNDARY_SENTENCES = Object.freeze([
  '[Introduction to Coherence](https://docs.oracle.com/en/middleware/fusion-middleware/coherence/12.2.1.4/develop-applications/introduction-coherence.html)只说明分区数据与再平衡；[Implementing Storage and Backing Maps](https://docs.oracle.com/middleware/1221/coherence/develop-applications/cache_back.htm)独立说明备份/存储、后备映射与外部持久存储机制。',
  '[Introduction to Coherence](https://docs.oracle.com/en/middleware/fusion-middleware/coherence/12.2.1.4/develop-applications/introduction-coherence.html)只用于交叉验证分区数据与再平衡；[Implementing Storage and Backing Maps](https://docs.oracle.com/middleware/1221/coherence/develop-applications/cache_back.htm)独立用于检查备份/存储、后备映射与外部持久存储边界。',
]);
...
function assertSpaceBasedArticle(source) {
  ...
  for (const sentence of ORACLE_BOUNDARY_SENTENCES) assert.equal(visibleBody.split(sentence).length - 1, 1, 'one exact Oracle evidence boundary: ' + sentence);
  assert.doesNotMatch(visibleBody, /Introduction to Coherence[^；。\n]*分区数据[、，]备份/iu, 'Introduction to Coherence never carries a positive backup/storage claim');
...
test('STY-13 article, governed sources, relations and current projection satisfy their contracts', async () => {
