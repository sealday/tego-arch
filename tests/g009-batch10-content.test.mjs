...
test('STY-09 Draw.io/SVG diagram requires real editable region, text, edge-label and legend counterparts', () => {
  ...
  for (const [role, count] of [['region', 5], ['region-title', 5], ['node-shape', 27], ['node-title', 27], ['node-type', 27], ['edge-label', 27], ['legend-card', 4], ['legend-anchor', 8], ['legend-caption', 8]]) assert.equal(roles.filter((candidate) => candidate === role).length, count, `${count} real Draw.io ${role} cells`);
...
test('STY-09 diagram inventory rejects editable counterpart removal and geometry/style drift', () => {
  ...
  for (const [label, id, from, to] of [
    ...
    ['node title baseline', 'title-node-order-input', 'baselineY=790', 'baselineY=791'],
