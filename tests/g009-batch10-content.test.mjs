...
export function parseDrawio(source) {
  return [...source.matchAll(/<mxCell\b[^>]*(?:\/>|>[\s\S]*?<\/mxCell>)/gu)].map(([raw]) => {
    ...
    return {name: 'mxCell', attributes: attributes(open), geometry: attributes(`<mxGeometry ${geometry}>`), points, hasPointsArray: Boolean(array), misplacedPoints, raw};
  });
}
...
function markerBox(svgNodes, path) { const marker = String(styleValue(path.attributes, 'marker-end') ?? '').match(/url\(#([^)]*)\)/u)?.[1]; assert.ok(marker, `${path.attributes.get('data-edge-id')} marker`); const markerNode = svgNodes.find(({name, attributes: a}) => name === 'marker' && a.get('id') === marker); assert.ok(markerNode, `${marker} marker definition`); assert.equal(markerNode.attributes.get('transform'), undefined, `${marker} marker transform flattened`); const width = number(markerNode.attributes.get('markerWidth'), `${marker} width`); const height = number(markerNode.attributes.get('markerHeight'), `${marker} height`); const viewBox = parseBounds(markerNode.attributes.get('viewBox') ?? `0 0 ${width} ${height}`, `${marker} viewBox`); const refX = number(markerNode.attributes.get('refX') ?? viewBox.width, `${marker} refX`); const refY = number(markerNode.attributes.get('refY') ?? viewBox.height / 2, `${marker} refY`); const markerPath = svgNodes.find(({name, attributes: a, index}) => name === 'path' && index > markerNode.index && index < markerNode.index + 4 && a.get('d')); assert.ok(markerPath, `${marker} real marker shape`); assert.ok(styleValue(markerPath.attributes, 'fill') && styleValue(markerPath.attributes, 'stroke'), `${marker} fill/stroke`); const glyph = parsePathPoints(markerPath.attributes.get('d')); const minX = Math.min(...glyph.map(({x}) => x)); const maxX = Math.max(...glyph.map(({x}) => x)); const minY = Math.min(...glyph.map(({y}) => y)); const maxY = Math.max(...glyph.map(({y}) => y)); const scaleX = width / viewBox.width; const scaleY = height / viewBox.height; const point = parsePathPoints(path.attributes.get('d')).at(-1); return {x: point.x + (minX - refX) * scaleX, y: point.y + (minY - refY) * scaleY, width: (maxX - minX) * scaleX, height: (maxY - minY) * scaleY}; }
function samePoint(left, right) { return Math.abs(left.x - right.x) < 0.01 && Math.abs(left.y - right.y) < 0.01; }
function assertRouteIntersections(routes) { for (let first = 0; first < routes.length; first += 1) for (let second = first + 1; second < routes.length; second += 1) { const [left, right] = [routes[first], routes[second]]; if (!routeIntersects(left.points, right.points)) continue; const allowed = [[left.points[0], right.points[0]], [left.points[0], right.points.at(-1)], [left.points.at(-1), right.points[0]], [left.points.at(-1), right.points.at(-1)]].some(([a, b]) => samePoint(a, b)); assert.ok(allowed, `${left.id}/${right.id} crossing or partial collinear overlap outside exact shared endpoint`); } }
...
export function assertDiagram(drawio, svg) {
  ...
  const edges = cells.filter(({attributes: a}) => a.get('edge') === '1'); assert.deepEqual(edges.map(({attributes: a}) => a.get('id')).sort(), CONNECTOR_INVENTORY.map(([id]) => id).sort(), 'exact Draw.io connector inventory'); assert.deepEqual(svgNodes.filter(({name, attributes: a}) => name === 'path' && a.get('data-edge-id')).map(({attributes: a}) => a.get('data-edge-id')).sort(), CONNECTOR_INVENTORY.map(([id]) => id).sort(), 'exact SVG connector inventory'); const routes = [];
  ...
  const viewBox = svgNodes.find(({name}) => name === 'svg')?.attributes.get('viewBox'); const width = parseBounds(viewBox, 'viewBox').width; const scale = 800 / width; for (const [id, box] of nodeBoxes) { const title = svgNodes.find(({name, attributes: a}) => name === 'text' && a.get('data-header-for') === id); const type = svgNodes.find(({name, attributes: a}) => name === 'text' && a.get('data-type-for') === id); assert.ok(title && type, `${id} requires title and type glyphs`); const titleX = number(title.attributes.get('x'), `${id} title x`); const titleY = number(title.attributes.get('y'), `${id} title y`); const typeX = number(type.attributes.get('x'), `${id} type x`); const typeY = number(type.attributes.get('y'), `${id} type y`); const titleSize = number(title.attributes.get('font-size'), `${id} title size`); const typeSize = number(type.attributes.get('font-size'), `${id} type size`); assert.ok(titleSize >= 15 && typeSize >= 15, `${id} body text >=15px`); assert.ok((titleX - box.x) * scale >= 16 && (typeX - box.x) * scale >= 16, `${id} glyph horizontal padding 16px`); assert.ok((titleY - titleSize - box.y) * scale >= 14, `${id} glyph vertical padding 14px`); assert.ok((box.y + box.height - (typeY + typeSize * 0.2)) * scale >= 14, `${id} text-to-bottom 14px`); assert.ok(Math.abs(typeY - titleY) * scale >= 22, `${id} title/type baseline 22px`); }
...
function fixtureDiagram() {
  ...
  const box = (id) => { const [x, y] = position.get(id); return {x, y, width: 160, height: 170}; };
  ...
  const edges = CONNECTOR_INVENTORY.map(([id, source, target]) => `<mxCell id="${id}" edge="1" source="${source}" target="${target}" style="exitX=1;exitY=0.5;entryX=0;entryY=0.5;"><mxGeometry relative="1" as="geometry"><Array as="points"></Array></mxGeometry></mxCell>`).join('');
  ...
  const shapes = DIAGRAM_NODES.map((id) => { const item = box(id); return `<rect data-node-id="${id}" x="${item.x}" y="${item.y}" width="${item.width}" height="${item.height}" fill="#FFFFFF" stroke="#0F172A"/><text data-header-for="${id}" x="${item.x + 48}" y="${item.y + 55}" font-size="15">${id}</text><text data-type-for="${id}" x="${item.x + 48}" y="${item.y + 120}" font-size="15">Filter</text>`; }).join('');
