...
test('STY-09 marker clearances use transformed painted geometry', () => {
  const {drawio, svg} = fixtureDiagram(); const markerPath = '<path class="marker-shape" d="M 0 0 L 15 7.5 L 0 15 z" stroke-width="3" stroke-linejoin="miter" stroke-miterlimit="4"/>';
  const harmless = replaceOnce(svg, markerPath, '<path class="marker-shape" transform="translate(0 0) rotate(0) scale(1)" d="M 0 0 L 15 7.5 L 0 15 z" stroke-width="3" stroke-linejoin="miter" stroke-miterlimit="4"/>', 'harmless marker transform'); assert.doesNotThrow(() => assertDiagram(drawio, harmless), 'supported no-op marker transforms are evaluated, not rejected');
  const polygon = replaceOnce(svg, markerPath, '<polygon class="marker-shape" points="0,0 15,7.5 0,15" stroke-width="3" stroke-linejoin="miter" stroke-miterlimit="4"/>', 'marker polygon'); assert.doesNotThrow(() => assertDiagram(drawio, polygon), 'supported marker child shapes are evaluated');
  for (const [name, mutation, expected = /complete CSS-pixel physical geometry/u] of [
    ['transform', replaceOnce(svg, markerPath, '<path class="marker-shape" transform="translate(0 100)" d="M 0 0 L 15 7.5 L 0 15 z" stroke-width="3" stroke-linejoin="miter" stroke-miterlimit="4"/>', 'marker child transform')],
