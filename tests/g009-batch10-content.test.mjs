...
function assertPhysicalGeometry(source, parsed, routes, nodeBoxes, viewBox, scale) { const failures = []; assertRouteIntersections(routes); for (const route of routes) { for (const [id, node] of nodeBoxes) if (![route.source, route.target].includes(id)) { if (routeRectangleDistance(route.points, expanded(node.bounds, node.stroke / 2)) === 0) failures.push(`${route.id} route/foreign-node ${id}`); if (rectangleDistance(route.marker.bounds, expanded(node.bounds, node.stroke / 2)) === 0) failures.push(`${route.id} marker/foreign-node ${id}`); } if (route.marker.bounds.left < viewBox.left || route.marker.bounds.right > viewBox.right || route.marker.bounds.top < viewBox.top || route.marker.bounds.bottom > viewBox.bottom) failures.push(`${route.id} marker/boundary`); }
  for (const route of routes) { const label = textBox(source, route.label); const strokeGap = Math.min(...routes.map((candidate) => routeRectangleDistance(candidate.points, label) - candidate.stroke / 2)) * scale; if (!(strokeGap >= 8)) failures.push(`${route.id} label/stroke ${strokeGap}`); const markerGap = Math.min(...routes.map((candidate) => rectangleDistance(candidate.marker.bounds, label))) * scale; if (!(markerGap >= 16)) failures.push(`${route.id} label/real-arrow ${markerGap}`); for (const [id, node] of nodeBoxes) if (![route.source, route.target].includes(id)) { const gap = rectangleDistance(label, expanded(node.bounds, node.stroke / 2)) * scale; if (!(gap >= 12)) failures.push(`${route.id} label/foreign-node ${id} ${gap}`); } }
  for (const [id, node] of nodeBoxes) { const title = parsed.elements.filter(({name, attributes: item}) => name === 'text' && item.get('data-header-for') === id); const type = parsed.elements.filter(({name, attributes: item}) => name === 'text' && item.get('data-type-for') === id); assert.equal(title.length, 1, `${id} mandatory title node`); assert.equal(type.length, 1, `${id} mandatory type node`); const texts = [title[0], type[0]]; const boxes = texts.map((element) => textBox(source, element)); for (let index = 0; index < texts.length; index += 1) { const finalFont = number(svgPresentationValue(source, texts[index], 'font-size'), `${id} body font`) * textScale(elementTransform(texts[index])) * scale; if (!(finalFont >= 15)) failures.push(`${id} body/font ${finalFont}`); const horizontal = Math.min(boxes[index].left - node.bounds.left - node.stroke / 2, node.bounds.right - node.stroke / 2 - boxes[index].right) * scale; if (!(horizontal >= 16)) failures.push(`${id} glyph/horizontal ${horizontal}`); } const top = (Math.min(...boxes.map((item) => item.top)) - node.bounds.top - node.stroke / 2) * scale; const bottom = (node.bounds.bottom - node.stroke / 2 - Math.max(...boxes.map((item) => item.bottom))) * scale; if (!(top >= 14)) failures.push(`${id} glyph/top ${top}`); if (!(bottom >= 14)) failures.push(`${id} glyph/bottom ${bottom}`); const titleBaseline = transformPoint(elementTransform(title[0]), {x: number(title[0].attributes.get('x'), `${id} title x`), y: number(title[0].attributes.get('y'), `${id} title y`)}); const typeBaseline = transformPoint(elementTransform(type[0]), {x: number(type[0].attributes.get('x'), `${id} type x`), y: number(type[0].attributes.get('y'), `${id} type y`)}); const baseline = Math.hypot(typeBaseline.x - titleBaseline.x, typeBaseline.y - titleBaseline.y) * scale; if (!(baseline >= 22)) failures.push(`${id} title/type baseline ${baseline}`); }
  assert.deepEqual(failures, [], `complete CSS-pixel physical geometry: ${failures.join(', ')}`); assertNoOverdraw(source, parsed, routes);
}
export function assertDiagram(drawio, svg) {
  assert.doesNotMatch(drawio, /(?:sourcePoint|targetPoint|dataRoute)/u, 'fallback route claims forbidden'); assert.doesNotMatch(svg, /(?:dataRoute|data-terminal)/u, 'SVG self-reported route claims forbidden'); const root = svg.match(/<svg\b[^>]*>/u)?.[0] ?? ''; assert.match(root, /role="img"/u, 'SVG image role'); assert.doesNotMatch(root, /(?:width|height)="/u, 'responsive SVG'); const viewBox = parseBounds(attributes(root).get('viewBox'), 'SVG viewBox'); const scale = 800 / viewBox.width; const parsedDrawio = parseDrawio(drawio); const parsedSvg = parseSvg(svg); const nodes = parsedDrawio.nodes.map(({attributes: item}) => item.get('id')); assert.deepEqual([...nodes].sort(), [...DIAGRAM_NODES].sort(), 'exact Draw.io node inventory'); const svgNodes = parsedSvg.nodes.filter(({name}) => name === 'g' || ['rect', 'path', 'polygon', 'circle', 'ellipse'].includes(name)); assert.deepEqual(svgNodes.map(({attributes: item}) => item.get('data-node-id')).sort(), [...DIAGRAM_NODES].sort(), 'exact SVG node inventory'); const nodeById = new Map(parsedDrawio.nodes.map((node) => [node.attributes.get('id'), node])); const nodeBoxes = new Map();
  for (const id of DIAGRAM_NODES) { const cell = nodeById.get(id); const node = svgNodes.find(({attributes: item}) => item.get('data-node-id') === id); assert.ok(cell && node, `${id} synchronized node`); const shape = nodeShape(parsedSvg, node); const actual = visibleShapeBounds(shape); const expected = bounds(cell); assert.deepEqual([actual.left, actual.top, actual.right, actual.bottom].map((value) => Math.round(value * 1e6) / 1e6), [expected.x, expected.y, expected.x + expected.width, expected.y + expected.height], `${id} actual painted bounds parity`); const stroke = number(svgPresentationValue(svg, shape, 'stroke-width') ?? 0, `${id} node stroke`); const title = parsedSvg.elements.find(({name, attributes: item}) => name === 'text' && item.get('data-header-for') === id); const type = parsedSvg.elements.find(({name, attributes: item}) => name === 'text' && item.get('data-type-for') === id); assert.ok(title && type, `${id} title/type text`); assert.equal(`${elementText(svg, title)}｜${elementText(svg, type)}`, cell.label, `${id} visible label synchronized with Draw.io`); const style = styleMap(cell.attributes.get('style')); assert.equal(svgPresentationValue(svg, shape, 'fill'), style.get('fillColor'), `${id} effective fill`); assert.equal(svgPresentationValue(svg, shape, 'stroke'), style.get('strokeColor'), `${id} effective stroke`); for (const text of [title, type]) { assert.equal(number(svgPresentationValue(svg, text, 'font-size'), `${id} text font`), number(style.get('fontSize'), `${id} Draw.io font`), `${id} font metric parity`); assert.equal(svgPresentationValue(svg, text, 'font-family'), style.get('fontFamily'), `${id} inherited text font family`); assert.equal(svgPresentationValue(svg, text, 'fill'), style.get('fontColor'), `${id} effective text color`); } nodeBoxes.set(id, {bounds: actual, stroke}); }
  const edges = parsedDrawio.edges; assert.deepEqual(edges.map(({attributes: item}) => item.get('id')).sort(), CONNECTOR_INVENTORY.map(([id]) => id).sort(), 'exact Draw.io connector inventory'); assert.deepEqual(parsedSvg.edges.map(({attributes: item}) => item.get('data-edge-id')).sort(), CONNECTOR_INVENTORY.map(([id]) => id).sort(), 'exact SVG connector inventory'); const routes = [];
  for (const [id, source, target, role] of CONNECTOR_INVENTORY) { const edge = edges.find(({attributes: item}) => item.get('id') === id); const path = parsedSvg.edges.find(({attributes: item}) => item.get('data-edge-id') === id); assert.ok(edge && path, `${id} synchronized connector`); assert.deepEqual([edge.attributes.get('source'), edge.attributes.get('target'), path.attributes.get('data-source'), path.attributes.get('data-target'), path.attributes.get('data-role')], [source, target, source, target, role], `${id} semantic parity`); const points = renderedPathPoints(path); equalRoute(points, drawioRoute(edge, nodeById), `${id} real route parity`); const expected = ROLE_STYLES[role]; assert.equal(svgPresentationValue(svg, path, 'stroke'), expected.stroke, `${id} effective stroke`); const stroke = number(svgPresentationValue(svg, path, 'stroke-width'), `${id} effective width`); assert.equal(stroke, expected.width, `${id} stroke width`); const dash = svgPresentationValue(svg, path, 'stroke-dasharray') ?? ''; assert.equal(dash === 'none' ? '' : dash, expected.dash, `${id} effective dash`); const label = parsedSvg.elements.find(({name, attributes: item}) => name === 'text' && item.get('data-edge-id') === id); assert.ok(label, `${id} visible label`); assert.equal(elementText(svg, label), edge.label, `${id} Draw.io/SVG label parity`); const edgeStyle = styleMap(edge.attributes.get('style')); assert.equal(svgPresentationValue(svg, label, 'font-family'), edgeStyle.get('fontFamily'), `${id} inherited label font family`); assert.equal(svgPresentationValue(svg, label, 'fill'), edgeStyle.get('fontColor'), `${id} effective label color`); assert.equal(number(svgPresentationValue(svg, label, 'font-size'), `${id} label source font`), number(edgeStyle.get('fontSize'), `${id} Draw.io label font`), `${id} label metric parity`); const finalFont = number(svgPresentationValue(svg, label, 'font-size'), `${id} label font`) * textScale(elementTransform(label)) * scale; assert.ok(finalFont >= 15, `${id} edge label final font ${finalFont}`); const marker = markerGeometry(svg, parsedSvg, path, points); for (const shape of marker.shapes) { assert.equal(svgPresentationValue(svg, shape, 'fill'), expected.stroke, `${id} marker effective fill`); assert.equal(svgPresentationValue(svg, shape, 'stroke'), expected.stroke, `${id} marker effective stroke`); } routes.push({id, source, target, role, points, path, marker, label, stroke}); }
  assertPhysicalGeometry(svg, parsedSvg, routes, nodeBoxes, viewBox, scale);
  const adjacency = new Map(); for (const route of routes) { const links = adjacency.get(route.source) ?? []; links.push(route.target); adjacency.set(route.source, links); } const reachesIrreversible = (start, visited = new Set()) => { if (['batch-published-output', 'stream-continuous-output'].includes(start)) return true; if (visited.has(start)) return false; visited.add(start); return (adjacency.get(start) ?? []).some((target) => reachesIrreversible(target, visited)); }; for (const start of ['rerun-partition', 'checkpoint-recovery', 'reconcile-authority', 'manual-terminal', 'bad-record-error', 'technical-failure-error', 'unknown-external-effect-error']) assert.equal(reachesIrreversible(start), false, `${start} recovery/replay cannot reach irreversible output`);
...
function fixtureDiagram() {
  ...
  const edges = CONNECTOR_INVENTORY.map(([id, source, target, role]) => { const style = ROLE_STYLES[role]; return `<mxCell id="${id}" value="流" edge="1" source="${source}" target="${target}" style="strokeColor=${style.stroke};strokeWidth=${style.width};dashed=${style.dash ? 1 : 0};dashPattern=${style.dash};endArrow=block;endFill=1;fontSize=45;fontFamily=Arial;fontColor=#111827;exitX=1;exitY=0.5;exitDx=0;exitDy=0;exitPerimeter=1;entryX=0;entryY=0.5;entryDx=0;entryDy=0;entryPerimeter=1;"><mxGeometry relative="1" as="geometry"><Array as="points"></Array></mxGeometry></mxCell>`; }).join('');
  const markerDefs = Object.keys(ROLE_STYLES).map((role) => `<marker id="arrow-${role}" class="marker marker-${role}" viewBox="0 0 12 12" refX="12" refY="6" markerWidth="12" markerHeight="12" markerUnits="userSpaceOnUse" orient="auto"><path class="marker-shape" d="M 0 0 L 12 6 L 0 12 z" stroke-width="1"/></marker>`).join('');
...
test('STY-09 marker clearances use transformed painted geometry', () => {
  const {drawio, svg} = fixtureDiagram(); const markerPath = '<path class="marker-shape" d="M 0 0 L 12 6 L 0 12 z" stroke-width="1"/>';
  const harmless = replaceOnce(svg, markerPath, '<path class="marker-shape" transform="translate(0 0) rotate(0) scale(1)" d="M 0 0 L 12 6 L 0 12 z" stroke-width="1"/>', 'harmless marker transform'); assert.doesNotThrow(() => assertDiagram(drawio, harmless), 'supported no-op marker transforms are evaluated, not rejected');
  const polygon = replaceOnce(svg, markerPath, '<polygon class="marker-shape" points="0,0 12,6 0,12" stroke-width="1"/>', 'marker polygon'); assert.doesNotThrow(() => assertDiagram(drawio, polygon), 'supported marker child shapes are evaluated');
  for (const [name, mutation, expected = /complete CSS-pixel physical geometry/u] of [
    ['transform', replaceOnce(svg, markerPath, '<path class="marker-shape" transform="translate(0 100)" d="M 0 0 L 12 6 L 0 12 z" stroke-width="1"/>', 'marker child transform')],
    ['reference point', replaceOnce(svg, 'refX="12"', 'refX="-1000"', 'marker refX')],
    ['reference y', replaceOnce(svg, 'refY="6"', 'refY="-1000"', 'marker refY')],
    ['viewBox', replaceOnce(svg, 'viewBox="0 0 12 12"', 'viewBox="0 0 1 1"', 'marker viewBox')],
    ['viewport', replaceOnce(replaceOnce(svg, 'markerWidth="12"', 'markerWidth="600"', 'marker width'), 'markerHeight="12"', 'markerHeight="600"', 'marker height')],
    ...
    ['painted geometry', replaceOnce(svg, 'd="M 0 0 L 12 6 L 0 12 z"', 'd="M 0 0 L 12 600 L 0 12 z"', 'marker painted path')],
