...
test('STY-09 Draw.io/SVG diagram binds actual marker dimensions', () => {
  const parsed = parseDrawio(file(DRAWIO)); for (const edge of parsed.edges) assert.equal(number(styleMap(edge.attributes.get('style')).get('endSize'), `${edge.attributes.get('id')} endSize`), 12, `${edge.attributes.get('id')} explicit marker size`);
});
...
test('STY-09 diagram inventory rejects editable counterpart removal and geometry/style drift', () => {
  ...
  for (const [label, id, from, to] of [
    ...
    ['node corner', 'node-order-input', 'arcSize=24', 'arcSize=23'],
    ['region corner', 'region-batch', 'arcSize=28', 'arcSize=27'],
    ['legend card corner', 'legend-card-data-flow', 'arcSize=24', 'arcSize=23'],
