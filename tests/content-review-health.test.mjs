...
test('loads non-empty canonical review inputs through generator plumbing', async () => {
  ...
  assert.equal(inputs.documents.length, 62);
  assert.equal(inputs.ledger.sources.length, 409);
  assert.equal(
  ...
  assert.equal(report.new_source_ids.length, 409);
  assert.equal(report.new_sources.length, 409);
  assert.equal(
