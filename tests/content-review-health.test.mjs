...
test('loads non-empty canonical review inputs through generator plumbing', async () => {
  ...
  assert.equal(inputs.documents.length, 83);
  assert.equal(inputs.ledger.sources.length, 459);
  assert.equal(
  ...
  assert.equal(report.new_source_ids.length, 459);
  assert.equal(report.new_sources.length, 459);
  assert.equal(
