...
test('loads non-empty canonical review inputs through generator plumbing', async () => {
  ...
  assert.equal(inputs.documents.length, 86);
  assert.equal(inputs.ledger.sources.length, 473);
  assert.equal(
  ...
  assert.equal(report.new_source_ids.length, 473);
  assert.equal(report.new_sources.length, 473);
  assert.equal(
