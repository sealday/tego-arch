...
function assertLayerContract(source) {
  ...
  assert.match(source, /表示层\s*-->\s*应用层/u);
  assert.match(source, /应用层\s*-->\s*领域层/u);
  assert.match(source, /领域层\s*-->\s*基础设施层/u);
  ...
  assert.match(source, /tier|部署单元/u);
  assert.match(source, /subgraph 单一部署单元/u);
...
test('preserves the Batch 2 STY-01 facts in the current Stage B projection', () => {
