...
export const STAGE_B_SOURCE_OBSERVED_AT = '2026-09-08T15:23:43.138Z';
...
export function assertBrowserEvidence(raw, {baseUrl = PRODUCTION_URL, expectedHead, expectedBuild, expectedPublication, expectedHttp, expectedSessionDiagnostics, expectedStateObservedAt, local = false, svgBytes, rawIdentity, rawBytes} = {}) {
  ...
  if (stageB) {
    assert.deepEqual(raw.sourceAudit, {scope:'one fresh session-wide source-anchor audit',observedAt:STAGE_B_SOURCE_OBSERVED_AT,fields:['text','href','target','rel'],destinationLoadsClaimed:false,anchors:STAGE_B_SOURCE_ANCHORS.map(([text,href]) => ({text,href,target:'_blank',rel:'noopener noreferrer'}))}, 'source audit records only fields actually observed');
...
export function stageBProductionFixture(svgBytes = readFileSync(SVG)) {
  ...
  const sourceAudit={scope:'one fresh session-wide source-anchor audit',observedAt:STAGE_B_SOURCE_OBSERVED_AT,fields:['text','href','target','rel'],destinationLoadsClaimed:false,anchors:STAGE_B_SOURCE_ANCHORS.map(([text,href]) => ({text,href,target:'_blank',rel:'noopener noreferrer'}))};
