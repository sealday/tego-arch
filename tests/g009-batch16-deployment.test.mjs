...
test('DDD-01 current local build identity includes sidebars.ts and matches recorded candidate inputs independently of observation head',()=>{const raw=JSON.parse(readFileSync(LOCAL_BROWSER));assert.deepEqual(currentBuildIdentity(),{inputFiles:raw.build.inputFiles,inputSha256:raw.build.inputSha256});assert.notEqual(execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim(),raw.build.baseHead,'observation base is not the post-commit HEAD');const paths=execFileSync('git',['ls-files','--cached','--others','--exclude-standard','-z',...BUILD_INPUTS],{encoding:'utf8'}).split('\0').filter(Boolean).sort();assert.throws(()=>currentBuildIdentity(paths.filter((p)=>p!=='sidebars.ts')),/sidebars\.ts/u);assert.notDeepEqual(currentBuildIdentity(paths,(p)=>p==='sidebars.ts'?Buffer.from('stale'):readFileSync(p)),{inputFiles:raw.build.inputFiles,inputSha256:raw.build.inputSha256});});
for(const [label,change] of [
