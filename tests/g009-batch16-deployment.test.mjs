...
export function assertBrowserEvidence(raw, {baseUrl = PRODUCTION_URL, expectedHead, expectedBuild, local = false, svgBytes, rawIdentity, rawBytes} = {}) {
  ...
  for (const [id,width,height,theme] of STATE_CONTRACTS) {
    ...
    const diagnostics = local
      ? {runtimeAndLog:{methods:['Runtime.exceptionThrown','Log.entryAdded'],afterSequence:157,cursor:181,events:[],hasMore:false,truncated:false},console:{methods:['Runtime.consoleAPICalled'],afterSequence:157,cursor:191,events:[],hasMore:false,truncated:false}}
      : {runtimeAndLog:{methods:['Runtime.exceptionThrown','Log.entryAdded'],afterSequence:22,cursor:182,events:[],hasMore:false,truncated:false},console:{methods:['Runtime.consoleAPICalled'],afterSequence:22,cursor:182,events:[],hasMore:false,truncated:false}};
    assert.deepEqual(s.diagnostics, diagnostics, 'fresh complete diagnostics for the selected observation environment');
