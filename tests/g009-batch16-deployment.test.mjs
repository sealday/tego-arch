...
export function assertBrowserEvidence(raw, {baseUrl = PRODUCTION_URL, expectedHead, expectedBuild, local = false, svgBytes, rawIdentity, rawBytes} = {}) {
  ...
  for (const [id,width,height,theme] of STATE_CONTRACTS) {
    ...
    for (const [i,w] of s.wrappers.entries()) {
      exactKeys(w, ['label','role','tabIndex','clientWidth','scrollWidth'], 'wrapper schema'); assert.equal(w.label, WRAPPER_LABELS[i]); assert.equal(w.role, 'region'); assert.equal(w.tabIndex, 0); assert.equal(w.clientWidth, width === 1440 ? 800 : 358); assert.ok(w.scrollWidth >= w.clientWidth);
    ...
    exactKeys(s.svg, ['complete','naturalWidth','naturalHeight','width','height','bytes','sha256'], 'loaded SVG schema'); assert.equal(s.svg.complete, true); for (const k of ['naturalWidth','naturalHeight','width','height']) assert.ok(s.svg[k] > 0); assert.deepEqual({bytes:s.svg.bytes,sha256:s.svg.sha256}, svgIdentity);
    ...
    exactKeys(s.diagnostics, ['afterSequence','cursor','events','hasMore','truncated'], 'complete diagnostics'); assert.deepEqual(s.diagnostics.events, []); assert.equal(s.diagnostics.hasMore, false); assert.equal(s.diagnostics.truncated, false); assert.ok(Number.isSafeInteger(s.diagnostics.afterSequence) && s.diagnostics.cursor > s.diagnostics.afterSequence); if (previousCursor !== null) assert.equal(s.diagnostics.afterSequence, previousCursor); previousCursor = s.diagnostics.cursor;
    assert.deepEqual(s.screenshot, {attempted:true,status:'BLOCKED',acceptance:'NOT_ACCEPTED',artifact:null}, 'no functional-to-visual promotion');
  }
...
export function browserFixture() {
  ...
  for (const [i,[id,width,height,theme]] of STATE_CONTRACTS.entries()) raw.states[id] = {url:baseUrl+ROUTE,h1:EXACT_METADATA.title,viewport:{width,height},theme,page:{clientWidth:width,scrollWidth:width},wrappers:WRAPPER_LABELS.map((label,j) => ({label,role:'region',tabIndex:0,clientWidth:width===1440?800:358,scrollWidth:j===0?800:1400})),interactions:WRAPPER_LABELS.map((label,j) => ({label,key:'ArrowRight',before:0,after:width===1440&&j===0?0:40,focused:true,focusVisible:true,outlineWidth:'3px',outlineStyle:'solid',method:'actual Tab/ArrowRight input; no synthetic event'})),svg:{complete:true,naturalWidth:1800,naturalHeight:1400,width:800,height:622,...svgAsset},relations:[{href:'/tego-arch/styles/sty-14',method:RELATION_METHOD,destination:{url:baseUrl+'/styles/sty-14',h1:'架构风格选择矩阵：边界、交互与演进触发器',returnHref:'/tego-arch/patterns/ddd-01'},returned:{url:baseUrl+ROUTE,h1:EXACT_METADATA.title,theme}}],sources:SOURCE_ANCHORS.map(([text,href]) => ({text,href,destination:href,rel:'noopener noreferrer',target:'_blank'})),ddd02ActionableCount:0,logs:[],diagnostics:{afterSequence:i*10,cursor:(i+1)*10,events:[],hasMore:false,truncated:false},screenshot:{attempted:true,status:'BLOCKED',acceptance:'NOT_ACCEPTED',artifact:null}};
...
for(const [state] of STATE_CONTRACTS) for(const [label,change] of [
  ['overflow',(s)=>{s.page.scrollWidth++;}],['lost focus',(s)=>{s.interactions[0].focused=false;}],['no scroll',(s)=>{s.interactions[1].after=0;}],['missing wrapper',(s)=>{s.wrappers.pop();}],['duplicate wrapper',(s)=>{s.wrappers[1]=structuredClone(s.wrappers[0]);}],['lost reciprocal',(s)=>{s.relations[0].destination.returnHref='/patterns';}],['false source destination',(s)=>{s.sources[0].destination='https://example.com/fake';}],['fabricated DDD-02',(s)=>{s.ddd02ActionableCount=1;}],['incomplete diagnostics',(s)=>{s.diagnostics.truncated=true;}],['console error',(s)=>{s.logs.push({level:'error',message:'failure'});}],['unloaded SVG',(s)=>{s.svg.complete=false;}],['visual PASS inflation',(s)=>{s.screenshot.status='PASS';s.screenshot.acceptance='ACCEPTED';}],
...
test('DDD-01 production Stage A Browser evidence is independently bound and semantic',()=>assertRecordedBrowserArtifact(STAGE_A_BROWSER,optionalText(REVIEW)));
}
