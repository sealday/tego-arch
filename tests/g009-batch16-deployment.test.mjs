...
export function assertBrowserEvidence(raw, {baseUrl = PRODUCTION_URL, expectedHead, expectedBuild, local = false, svgBytes, rawIdentity, rawBytes} = {}) {
  ...
  for (const [id,width,height,theme] of STATE_CONTRACTS) {
    ...
    assert.deepEqual(s.screenshot, {attempted:true,bytes:427000,status:'BLOCKED',acceptance:'NOT_ACCEPTED',artifact:null,reason:'fullPage capture returned bytes in the Browser session but could not be persisted as a durable artifact'}, 'no functional-to-visual promotion');
  }
...
export function currentBuildIdentity(paths = execFileSync('git', ['ls-files','--cached','--others','--exclude-standard','-z',...BUILD_INPUTS], {encoding:'utf8'}).split('\0').filter(Boolean).sort(), readInput = (path) => readFileSync(path)) {
  ...
  return {baseHead: execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim(), inputFiles: paths.length, inputSha256: hash(paths.map((path) => `${path}\0${hash(readInput(path))}\n`).join(''))};
}
...
test('DDD-01 current local build identity includes sidebars.ts and matches recorded candidate',()=>{const raw=JSON.parse(readFileSync(LOCAL_BROWSER));assert.deepEqual(currentBuildIdentity(),raw.build);const paths=execFileSync('git',['ls-files','--cached','--others','--exclude-standard','-z',...BUILD_INPUTS],{encoding:'utf8'}).split('\0').filter(Boolean).sort();assert.throws(()=>currentBuildIdentity(paths.filter((p)=>p!=='sidebars.ts')),/sidebars\.ts/u);assert.notDeepEqual(currentBuildIdentity(paths,(p)=>p==='sidebars.ts'?Buffer.from('stale'):readFileSync(p)),raw.build);});
for(const [label,change] of [
