...
控制流为：外部驱动方经输入适配器跨越核心拥有的输入端口，再进入提交订单用例；用例调用领域规则，并通过库存端口与订单仓储端口使用外部能力；外侧适配器实现这些端口。输入端口是边界合同，提交订单用例是实现该应用行为的 Application Service / Interactor，两者不得合并为一个节点。
...
主阅读方向从左到右：`外部驱动方 → 应用核心 → 外部机制`。应用核心内部把核心拥有的 `输入端口`、实现行为的 `提交订单用例` 和 `订单领域规则` 分开。库存、仓储端口位于核心边界，外部适配器位于边界外。

图必须表达：

- 控制流从驱动方进入核心，并由核心调用外部能力；
- 运行时控制必须先跨越 `n-input-port`，再进入 `n-usecase`；输入适配器的源码依赖 `d1` 只指向 `n-input-port`；
- 源码依赖在边界上指向内侧拥有的端口；
...
精确拓扑库存为九个语义节点、三个外边界和十一条关系：运行时 `c1 driver→input-adapter`、`c2 input-adapter→input-port`、`c8 input-port→usecase`、`c3 usecase→domain`、`c4 usecase→inventory-port`、`c5 inventory-port→inventory-adapter`、`c6 usecase→order-port`、`c7 order-port→database-adapter`；源码依赖 `d1 input-adapter→input-port`、`d2 inventory-adapter→inventory-port`、`d3 database-adapter→order-port`。`n-usecase` 的角色固定为 `Use Case / Application Service / Interactor`。

### 8.3 几何与可读性

Stage A 架构复审确认：输入边界与用例实现必须显式分离。以下几何增加第九个 `n-input-port` 边界节点，保留 `1200 × 760` viewBox、三个外边界和既有业务语义，并把关系库存更新为 `c1`–`c8` / `d1`–`d3`。SVG 不得使用 `textLength`、`lengthAdjust`、`spacingAndGlyphs` 或等价字形压缩，完整角色标签必须按自然字宽换成两至四行。
...
- 实施测量使用保守安全目标：节点水平内边距至少 `17 CSS px`，顶部和底部净空至少 `15 CSS px`；
- 不透明标签背景不得擦除连接线；
- 不得仅用颜色区分边界或关系。

图内必须可见声明 `术语映射用于对照，不表示节点完全等价`。实现阶段必须测量所有九个语义节点的最终 CSS 像素文本、基线和边缘净空。
