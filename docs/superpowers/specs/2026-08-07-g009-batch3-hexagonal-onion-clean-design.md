...
实施复审确认：原计划的窄节点无法同时容纳完整英文角色标签、`10 CSS px` 可读字号、`16 / 14 CSS px` 内边距和 `22 CSS px` 标题/类型基线距离。以下修订几何替代原窄节点几何；只放大八个语义节点，`1200 × 760` viewBox、三个外边界、语义节点集合和 `c1`–`c7` / `d1`–`d3` 拓扑不变。SVG 不得使用 `textLength`、`lengthAdjust`、`spacingAndGlyphs` 或等价字形压缩，完整角色标签必须按自然字宽换成两至四行。
