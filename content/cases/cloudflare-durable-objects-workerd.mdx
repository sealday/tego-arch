...
confidence: high
summary: 以智能体（Agent）状态为例，分析 Cloudflare 云平台（Cloudflare）中的 Cloudflare 持久对象（Durable Objects）与 workerd 开源运行时（workerd）如何用节点本地数据库（SQLite）、超文本传输协议（Hypertext Transfer Protocol，HTTP）和重试（Retry）机制承载边缘分片。
...
**已证实事实**：`workerd` 是与 Cloudflare 工作进程（worker）同源的开源 JavaScript 语言（JavaScript）与网页汇编服务端运行时。它可用于自行托管和本地开发，但说明文件警告它不是安全加固沙箱。固定`workerd.capnp`说明本地持久对象当前位于单个运行时实例；跨集群分布仍未实现。
