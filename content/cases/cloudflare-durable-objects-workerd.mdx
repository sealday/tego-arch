---
title: Cloudflare Durable Objects + workerd 边缘协调方案：把边缘协调收敛到身份寻址的状态单元
...
[STY-11 Serverless Architecture](/styles/sty-11) 只有限复用这里的身份寻址、单写者与热点判断；STY-11 的订单进度由独立持久工作流拥有，不能把 Cloudflare 持久对象的托管机制外推为所有函数运行时的状态、并发或恢复保证。
