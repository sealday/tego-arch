---
title: New API Channel Pool Routing：把模型请求分解为能力、层级与故障隔离
sidebar_label: New API Channel Pool Routing
...
confidence: high
summary: 用 Ability 索引、优先级层、权重选择和多 Key 状态分析统一模型网关如何路由、重试与隔离故障。
...
flowchart TD
    ...
    UP -->|"成功"| OK
    UP -->|"失败"| ERR
    ERR -->|"非 channel error 且 skip-retry / 不可重试"| STOP
    ERR -->|"渠道错误或配置命中"| ISO
    ERR --> RETRY
    ISO --> RETRY
    RETRY -->|"是"| P
...
先把选择过程拆成两个池。渠道池负责资格、优先级和权重；Key 池只在已选渠道内部过滤可用凭据。

<div
  className="architecture-diagram-scroll"
  role="region"
  aria-label="New API 两级候选池路由图，可横向滚动"
  tabIndex={0}
>

![请求经过 Ability、优先级和权重选择渠道，再进入渠道内部 Key 池](/img/diagrams/new-api-two-stage-routing.svg)
...
<div
  className="architecture-diagram-scroll"
  role="region"
  aria-label="New API 错误隔离与恢复图，可横向滚动"
  tabIndex={0}
>
...
<details className="evidence-card">
  <summary>证据：Ability、渠道选择与请求重试源码接缝</summary>
...
<details className="evidence-card">
  <summary>证据：多 Key 隔离、渠道禁用与受控恢复</summary>
...
**已证实事实**：固定提交将 `(group, model, channel ID)` 物化为 Ability，并让渠道状态联动 Ability 的启停。候选选择先按优先级确定层级，再在同层按权重随机；渠道内则从已启用 Key 中选择，单 Key 失效与渠道整体禁用分别维护。禁用状态影响后续候选资格，当前请求是否重试由另一段错误处理流程决定。

**个人分析**：基于这些机制，可以把本节的取舍归纳为四个架构决策；“第一个”到“第四个”是本文的分析框架，不是 New API 官方给出的决策编号。
...
**已证实事实**：固定提交会从渠道的 group/model 组合生成 Ability；渠道状态更新会联动 Ability，内存缓存只索引启用渠道，并按 group/model 维护候选渠道 ID。

**基于证据的推断**：Ability 是数据库派生资格，内存缓存又是第二份运行时索引，因此生产环境需要验证两者的一致性。渠道更新 group、model、priority、weight 或状态后，两份索引应在可接受窗口内收敛；监控需要区分“无 Ability”“Ability 已禁用”“缓存尚未刷新”和“选中后上游失败”。

**个人分析**：路由观测至少记录请求的 group、原始模型、规范化模型是否命中、目标优先级、渠道 ID、retry index 与结果分类。多 Key 只记录不可逆的 Key 指纹或索引，不能记录完整凭据；禁用原因也应经过敏感信息裁剪。
...
**个人分析**：故障注入应覆盖最高优先级返回可重试错误、同层渠道权重全零、单 Key 或全部 Key 自动禁用。还应覆盖手工禁用对象进入测试周期，以及响应成功但本地测试失败。每项都分别断言当前请求终态、未来候选集与审计记录。
