...
Action server 由应用决定是否接受 Goal 和 Cancel。接受 Cancel 只表示服务器会尝试取消；执行代码仍须观察取消状态、停止工作并提交 canceled 终态。
...
<details className="evidence-card">
  <summary>证据：固定 Jazzy 版本与生命周期文档边界</summary>
...
先沿长任务主链检查控制权。Lifecycle manager 决定节点是否参与处理，Action server 管理 goal 与 cancel，设备真实状态仍由驱动器回执和安全链确认。

<div
  className="architecture-diagram-scroll"
  role="region"
  aria-label="ROS 2 长任务控制合同图，可横向滚动"
  tabIndex={0}
>

![Lifecycle manager 激活 Managed Agent，Action server 管理任务，设备真实状态由驱动器确认](/img/diagrams/ros2-task-control-contract.svg)

</div>

取消路径尤其不能缩写成一个状态位。接受请求后，应用仍要停止命令、等待设备回执，再提交 canceled、failed 或人工终态。

<div
  className="architecture-diagram-scroll"
  role="region"
  aria-label="ROS 2 取消到物理终态图，可横向滚动"
  tabIndex={0}
>

![Cancel Request 被接受后，执行代码停止与撤销，等待设备回执后提交真实终态](/img/diagrams/ros2-cancel-physical-terminal.svg)

</div>

独立安全控制器仍可绕过这条软件取消链直接约束设备；Action 的成功或取消不能替代安全停机证明。
...
Reentrant Callback Group 允许组内与自身 callback 并行，Mutually Exclusive 禁止组内并行。前者只是许可，后者也只是进程内调度约束；实际并发仍取决于 Executor、线程、锁、其他 ready work 与 OS 调度。
...
<details className="evidence-card">
  <summary>证据：Executor 与 Callback Group 的固定源码接缝</summary>
...
<details className="evidence-card">
  <summary>证据：Action 取消与终态的固定源码接缝</summary>
...
<details className="evidence-card">
  <summary>证据：Lifecycle callback 与错误路径的固定源码接缝</summary>
...
**证据边界说明**：Agent 通信映射、业务 authority、细粒度授权、资源预算、幂等和 Cancel fencing 属于架构推断。独立安全回路、观测字段与恢复 runbook 也不是 ROS 2 的自动保证。
