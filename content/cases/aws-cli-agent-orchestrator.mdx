...
工具限制由配置档案映射到提供方机制。部分提供方是硬限制，Codex/Kimi 固定版本为提示级软限制；命令行智能体编排器MCP 工具也不能逐个阻断，`--yolo`会移除限制。
...
HTTP 默认绑定`127.0.0.1:9889`，开放授权协议未配置身份提供方时关闭。启用后验证RS256、签发方、受众和到期时间，并按`cao:read/write/admin`做路由门禁。
...
2. [`CODEBASE.md`](https://github.com/awslabs/cli-agent-orchestrator/blob/bae80071a17e001380367c461b32d64bc6b54433/CODEBASE.md)：沿命令行界面、MCP、FastAPI、服务、终端复用器、SQLite 数据库与提供方这条路径阅读。收件箱消息流程和移交流程给出比架构图更精确的调用链。
...
以下来源均为官方或上游材料。GitHub 链接固定到本文核对的提交，便于日后复现；网页规范可能继续演进。
