...
sidebar_label: Microsoft 多智能体参考架构
...
confidence: high
summary: 从注册、编排、记忆到治理，分析企业级多智能体平台如何约束自治边界。
series: ai-native
catalog_order: 1
featured: true
source_kinds:
...
**已证实事实**：仓库把自己定义为概念指南，关注多智能体编排、治理和扩展，而不是单个智能体实现。主参考图采用中央 Orchestrator。它同时配合 Classifier、Agent Registry、Supervisor、专业智能体、知识与持久化层及 MCP 集成层。通信、可观测性、评估、安全和治理则由专题章节补充。
...
Building Blocks 把 Orchestrator、Specialized Agents 和 Agent Registry 列为核心构件。它默认建议专业智能体经编排器通信。高度耦合的一组智能体可以封装为组合智能体。通信章节也允许请求式、消息驱动式及混合交互。
...
这张原创信息图负责建立控制顺序，不把 `ToolPolicyGateway` 描述成 Microsoft 产品，也不承担精确组件拓扑。它强调的判断是：受控自治来自可执行门禁，而不是在系统边缘附加一份治理报告。

再看下面的确定性组件图，区分“框架内”和“框架外”的职责。Orchestrator 与 Supervisor 可以由框架实现；Registry、策略、工具网关、审计和发布门禁却必须形成跨运行的一致合同。该图是对官方构件的平面化重绘，不是逐像素复制。
...
sequenceDiagram
  ...
  participant G as 应用 ToolPolicyGateway
...
最短路径先用 Building Blocks 确认最小角色，再用 Reference Architecture 跟踪端到端序列。最后用 Security/Governance 检查哪些控制不能交给模型。

只有准备拆服务或开放直连时，再继续阅读设计选项、通信、Registry、Memory、Observability 和 Evaluation。

将文档映射到代码时，先形成注册、任务信封、状态、策略决策、工具网关、遥测和评估七类合同。接口名可变，但任务信封必须携带租户、关联 ID、行为包版本、截止时间和授权上下文；否则审计与恢复会失去共同主键。
...
<details className="evidence-card">
  ...
  - **边界：** 这些名称是本文的实现映射，不是 Microsoft 仓库发布的 SDK 接口。
</details>
...
- **主体与范围：** 用户、工作负载身份、租户、关联 ID 和任务 ID。
...
以下均为官方或上游来源。访问日期与来源截断日期：**2026-07-24**。核对时，官方仓库 HEAD 仍为本文固定提交。
