...
**已证实事实与实现边界**：MCP 客户端与服务端标准化能力发现、协商与调用，不自动替应用完成最终用户重新鉴权、业务授权、限流或审计。本文的`ToolPolicyGateway`是应用控制点；它可以包裹MCP 客户端，也可以保护非MCP 工具。

**个人分析**：框架选择应跟随右侧这些约束，而不是反过来。跨团队、跨安全域、要求审计和独立版本治理时，控制平面的收益可能覆盖额外跳数、费用和故障面；若单智能体加工具已经可靠，最低必要复杂度仍是更好的起点。

## 事实边界

事实范围先回答“这份参考架构能证明什么”。它能证明职责、模式和官方建议，不能证明某个SDK、云服务或生产拓扑已经替项目完成这些职责。
...
<details className="evidence-card">
  ...
  - **仓库形态：** `docs/`、`components/`、Draw.io/SVG 图、文档构建工具、站点构建文件与文档脚本。
...
flowchart TB
  ...
  subgraph Execution["执行平面"]
    ...
    MCPClient["MCP客户端：能力协商与调用"]
    MCPServer["MCP服务端：暴露能力"]
    Tools["外部工具与服务"]
    Supervisor --> A1
    Supervisor --> A2
    A1 --> Gateway
...
这里的“源码导读”不是逐函数追踪。固定仓库的核心资产是标记文档、架构图和站点辅助文件；阅读目的，是把参考职责转换成所选SDK 或平台必须实现的接口合同。
...
<details className="evidence-card">
  ...
  - **边界：** 这些名称是本文的实现映射，不是该仓库发布的SDK 接口。
</details>
...
<details className="evidence-card">
  ...
  - **边界：** 这些文档证明参考职责，不证明任何接口名、部署拓扑或SDK 默认行为。
</details>
...
框架无关的是职责与质量约束：能力发现、任务分解、状态持久化、通信契约、最小权限、端到端追踪、离线与在线评估、生命周期治理和故障隔离。仍需选择的是SDK、模型、分类器、存储、代理节点、MCP与智能体间协议（Agent2Agent Protocol，A2A）的使用范围、部署平台、租户隔离级别、评估器、策略引擎、审批流程以及服务等级目标。参考架构提供问题清单，不替代架构决策记录（Architecture Decision Record，ADR）和威胁模型。
...
1. **先定义控制边界，再选择框架。** 把准入、路由、记忆、工具权限和发布门禁写成接口与ADR，避免被SDK 默认值反向塑造架构。
