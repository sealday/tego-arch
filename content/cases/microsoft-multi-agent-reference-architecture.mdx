...
[编排者–工作者与扇出和扇入](/patterns/agt-p-07)把这种分解、并行调度与有界汇聚整理为通用控制合同；本案例中的参考构件只提供实现证据，不证明所有任务都适合并行。
...
[多智能体研究系统参考设计](/cases/multi-agent-research-system)进一步展示一个作者设计的证据合成拓扑，用任务账本、证据门和引文核验约束研究扇出；它不是 Microsoft 参考架构已部署或已验证的组成部分。
