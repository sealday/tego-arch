...
<div className="table-wrapper table-wrapper--mapping diagram-wrapper--scroll-owner" role="region" aria-label="四层责任合同与八维架构剖面，可横向滚动" tabIndex={0} onKeyDown={handleHorizontalArrowKey}>
...
先记录现有跨层依赖与唯一写入路径，再把协议、业务判断和厂商类型移回其责任层。若查询开始参与权限判断或状态转换，撤销旁路。若大量业务变化持续横跨所有技术层，按业务能力重组 module/package，并重新比较边界、部署和团队所有权；依赖反转变体属于后续主题，本页不提供可执行链接。
