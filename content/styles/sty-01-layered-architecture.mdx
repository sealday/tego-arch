...
Layered Architecture 中的层是责任边界和依赖边界，不是目录、进程、服务、物理 tier、部署单元或团队的别名。[风格目录](/styles)提供主题入口；本文把[架构风格比较框架](/styles/sty-00)的八维剖面落实为一个可检查的经典分层合同。

## 学习问题

- 如何区分逻辑 layer、代码 module/package 与物理 tier 或部署单元？
...
逻辑层可由代码 module 或 package 实现；物理 tier 或部署单元则是构建、发布、扩缩与故障传播边界。二者可以重合，却不能从层名自动推导。
...
封闭层默认只允许紧邻向下依赖。领域层依赖基础设施层公开的稳定数据访问能力合同，但不能接收其 schema、ORM、连接配置或厂商类型。
...
领域规则决定订单和库存状态变化是否合法，基础设施层实现持久化。订单确认与库存预留保持在同一本地事务中；报表查询只能读取投影，不得改变权威状态、重算业务不变量或返回 ORM 与数据库类型。
...
先记录现有跨层依赖与唯一写入路径，再把协议、业务判断和厂商类型移回其责任层。若查询开始参与权限判断或状态转换，撤销旁路。若大量业务变化持续横跨所有技术层，按业务能力重组 module/package，并重新比较边界、部署和团队所有权；依赖反转变体可继续参阅[Hexagonal、Onion 与 Clean Architecture 对照](/styles/sty-02)。
...
[生产中的 Micro-Frontend 案例](/cases/micro-frontends-single-spa)展示了另一种边界压力：按技术层组织的共同变更与按用户价值形成垂直切片，会带来不同的部署和团队权衡。该案例用于比较边界，不把前端部署方式移植为本订单系统的默认方案。

## 来源

[Microsoft N-tier architecture style](https://learn.microsoft.com/en-us/azure/architecture/guide/architecture-styles/n-tier)用于区分逻辑 layer 与物理 tier，并支持开放层、封闭层和向下依赖的事实边界；本文不采用其 Azure 部署拓扑。[Martin Fowler 的 Presentation Domain Data Layering](https://martinfowler.com/bliki/PresentationDomainDataLayering.html)用于核对表示、领域、数据责任分离和逻辑层不等于部署单元。[AWS Hexagonal Architecture Overview](https://docs.aws.amazon.com/prescriptive-guidance/latest/hexagonal-architectures/overview.html)只用于提示经典分层与依赖反转的差异，不在本文展开后者。[ArchUnit User Guide](https://www.archunit.org/userguide/html/000_Index.html)说明层间访问和循环约束可以成为自动化架构规则；Java 与 ArchUnit 只是实现示例，不是本文要求。

本文的责任合同、八维剖面、例外表、订单演练和 Mermaid 均为 Tego Arch 原创综合，不复制来源图示、表格、结构或长段落。
