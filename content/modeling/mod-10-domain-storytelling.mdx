...
- 如何选择故事的粒度、as-is/to-be 与 pure/digitalized scope，又如何先完成典型路径、将重要变体分离为新的 Domain Story？
...
输入应包含一个真实但已脱敏的典型实例，并明确粒度、as-is/to-be、pure/digitalized、开始点、典型路径结束条件、非目标、已知分歧与假设，以及重要变体另建 Domain Story 的规则。参与者可以带来操作说明、页面、请求记录与回执样例，但这些材料只是澄清叙述的证据，不能预先决定故事句子。
...
Domain Story 中发现的语言分歧、权威记录冲突和协作变化只能作为后续输入。正式系统边界与 Bounded Context 仍需交给 [MOD-11 DDD Context Map 建模](/modeling/mod-11)或等价架构活动验证；actor 与工作对象不能直接生成 Context。
...
- [How to Model Repeating Activities](https://domainstorytelling.org/articles/how-to-model-loops/)（Stefan Hofer）：支持用具体实例、annotation 与 group 表达重复活动的选择；不赋予 Domain Story 正式循环、分支或执行语义。
