...
- [Domain Storytelling Quick-Start Guide](https://domainstorytelling.org/quick-start-guide)：支持 actor、work object、activity、sequence number、annotation、逐句复述与 scope 因子的定义；不证明本文费用支付故事或任何生产实现。
- [The First Book About Domain Storytelling](https://domainstorytelling.org/book)：提供方法、场景建模、工作坊和与其他建模方法关系的学习入口；不授权把示例、图或文字复制为本站内容。
