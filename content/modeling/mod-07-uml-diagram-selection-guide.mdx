...
[MOD-01 模型选择总览](/modeling/mod-01)提供从问题到模型的总入口；[MOD-03 C4 Dynamic 与 Deployment](/modeling/mod-03)用于比较单场景协作和部署实例边界；[MOD-06 ER 模型与关系边界](/modeling/mod-06)帮助区分 UML class 与数据模型。完整状态机设计属于下一主题，但 MOD-08 仍未发布，因此这里不建立无效链接。
