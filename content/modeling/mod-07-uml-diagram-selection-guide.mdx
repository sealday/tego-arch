---
title: 统一建模语言（Unified Modeling Language，UML）选图指南
...
tags:
  ...
  - 模型选择
  - 证据边界
summary: 从评审问题选择用例、时序图、状态、类或部署，并明确每类图能证明和不能证明什么。
topic_id: MOD-07
priority: P0
depends_on:
  - MOD-01
...
- 如何从当前评审问题出发，在用例、时序图、状态、类与部署中选择最小而充分的 UML 视图？
- 五类图各自观察什么对象和时间尺度？
...
[MOD-02 C4 系统上下文图与容器](/modeling/mod-02)中的名称和系统边界是本文的权威输入；本文的选图矩阵、问题、场景细节和预期判断都是本站原创的教学假设，不代表现有系统已经如此实现或验证。
...
- 默认画类图，把熟悉的工具误当成问题分类。
- 用时序图表示静态所有权，或把消息顺序误读为性能时限、全局并发顺序和事务保证。
- 用状态图暗示跨对象原子性或分布式一致性。
- 把 UML 类当成实体关系实体、逻辑数据模型或物理数据库表；UML 类不是这些数据产物的替代物。
- 用部署图声称实际库存、容量、故障切换或安全控制已经验证。
...
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
[MOD-01 模型选择总览](/modeling/mod-01)提供从问题到模型的总入口；[MOD-03 C4 动态图与部署](/modeling/mod-03)用于比较单场景协作和部署实例边界；[MOD-06 实体关系模型与关系边界](/modeling/mod-06)帮助区分 UML 类与数据模型；[MOD-08 状态机建模](/modeling/mod-08)继续展开单对象生命周期、转换证据与禁止推断边界。
...
**预期判断：** 选择用例，评审参与者、业务目标、系统边界与外部交互范围是否完整且用词一致。
...
**预期判断：** 选择时序图，评审场景参与者、消息相对顺序、同步点、批准分支和拒绝分支是否支持这一次交互说明。

**证据缺口：** 延迟目标、并发提交、消息重试（Retry）和事务边界仍需运行追踪、负载、并发与故障测试证明。
...
**预期判断：** 选择状态，评审申报能否从允许的事件和守卫到达合法状态，并识别不允许的转换。
...
**预期判断：** 选择类，评审类型职责、关联、多重性和必要泛化是否足以表达静态协作结构；UML 类不是实体关系实体，也不是数据库表。

**证据缺口：** 运行时创建数量、生命周期事实、持久化模式和具体代码归属仍需代码、数据模型与运行检查证明。

### 部署：指定环境中的运行位置

**评审问题：** 费用申报网页应用、申报应用程序编程接口（Application Programming Interface，API）、支付任务执行器、申报数据库和银行支付服务在指定环境中如何部署？

{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
**输入事实：** 采用 MOD-02 的系统边界和银行支付服务权威名称，把指定环境、节点、执行环境、部署单元与通信路径作为待核对输入。

{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
**预期判断：** 选择部署，评审各软件实例运行位置、节点边界和必要通信路径是否清晰，并与 MOD-03 的部署观察尺度保持一致。
...
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
- [C4 Deployment diagram](https://c4model.com/diagrams/deployment)只支持指定环境中的部署节点、基础设施节点、软件系统实例与容器实例范围；它不证明生产库存、容量、故障切换或运行健康。
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
