...
[MOD-08 状态机建模](/modeling/mod-08)约束未知支付结果与证据收敛；[MOD-09 EventStorming](/modeling/mod-09)提供事件、政策、热点和候选边界线索；[MOD-10 Domain Storytelling](/modeling/mod-10)提供具体场景中的语言、工作对象与协作变化。[MOD-12 架构图审阅清单](/modeling/mod-12)只能接收这里仍标为候选、且带有非证明边界的产物，并保留“candidate Context is not component decomposition（候选 Context 不是组件拆分）”这一交接约束。
...
- [Bounded Context](https://martinfowler.com/bliki/BoundedContext.html)：支持语言与模型边界以及显式 Context Map 关系；不证明本文候选边界、服务、团队或运行时设计。
- [DDD Crew Context Mapping](https://github.com/ddd-crew/context-mapping/tree/970c1ff3a61f7aa8b61b789b697c05bc585f614d)：支持围绕具体问题绘制小型 Context Map、逐关系判断 U/D 与关系模式的存在；不复用其 cheat sheet 或 Miro 资产，也不替本文选择关系模式。
- [Anti-Corruption Layer](https://contextmapper.org/docs/anticorruption-layer/)：支持 ACL 作为下游翻译与隔离责任；不证明银行边界已有 ACL、OHS/PL、协议或部署实现。
- [Avanscoperta Context Mapping](https://www.avanscoperta.it/en/context-mapping/)：支持边界指标并非绝对、仍需架构判断；不批准本文候选边界。
