...
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
全景是发现全局叙事、关键转折、热点和未知项的工作坊格式。过程建模是围绕一个具体流程连接人员、读模型、命令、系统、事件与策略的工作坊格式。Software Design 是在更窄范围内探索实现协作与设计选择的工作坊格式。三者服务于不同讨论尺度，不是正式层级，也不会自动把上一轮贴纸变成下一轮架构决定。
...
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
[Temporal Saga 与持久执行案例](/cases/temporal-saga-durable-execution)可用于检验长时流程中的超时、重试、补偿和人工收敛，但其运行机制不会替本文决定业务边界。候选关系需在 [MOD-11 领域驱动设计（Domain-Driven Design，DDD）上下文映射建模](/modeling/mod-11)或等价架构活动中结合术语负责人、规则变更、权威记录和组织协作证据继续验证。事件风暴的泳道、热点与边界信号不能直接生成上下文。
