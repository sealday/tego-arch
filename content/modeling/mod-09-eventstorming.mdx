...
3. 补充事件来源、参与者、外部系统、pivotal event 候选、hotspot 和未知项。
4. 共同 walkthrough，合并同义词，同时保留真实分歧。
5. 选择“费用已审批到支付结果已确认”的高风险片段。
6. 用 Person、Read Model、Command、System、Policy 和 Event 建立 Process Model。
7. 回到 hotspot，将事项记录为已回答、待验证或超出本轮范围。
...
[MOD-10 Domain Storytelling](/modeling/mod-10)与 EventStorming 可以组合，但不是替代关系，两种协作方法之间没有严格的元素映射。

[Temporal Saga 与持久执行案例](/cases/temporal-saga-durable-execution)可用于检验长时流程中的超时、重试、补偿和人工收敛，但其运行机制不会替本文决定业务边界。候选关系需在 [MOD-11 DDD Context Map 建模](/modeling/mod-11)或等价架构活动中结合术语 owner、规则变更、权威记录和组织协作证据继续验证。EventStorming 的 lane、热点与边界信号不能直接生成 Context。
...
- [Collaborative Process Modelling with EventStorming](https://medium.com/@ziobrando/collaborative-process-modelling-with-eventstorming-17ed363650c0)：支持 Process Modelling 工作坊格式中的 Person、System、Command、Policy、Read Model 与 Event 语法；不定义本文的费用申报案例或架构。
- [Pivotal Events](https://www.avanscoperta.it/en/eventstorming/pivotal-events/)：支持 pivotal event、时间线与 swimlane 的讨论信号；不把 pivotal event 或 swimlane 变成 context、团队、系统或服务。
