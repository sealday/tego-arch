...
MOD-02 只提供费用申报系统的权威边界与 `银行支付服务` 名称；它不证明本页的数据设计。MOD-05 提供 `Employee`、`ExpenseClaim`、`Approval` 和 `PaymentInstruction` 四个起始逻辑实体。本页的六实体模型、字段、基数、半开区间和不变量都是本站原创的教学决定，不是已被证明的生产事实。
...
MOD-01 负责按评审问题选择模型；MOD-05 为本页提供四个核心逻辑实体；PR-13 帮助区分领域含义与持久化机制。Temporal Saga 案例中的 Event History 是控制面恢复记录，不是费用申报的业务关系历史。MOD-08 未发布，将来可用状态机补充批准与付款前置条件；本页不创建尚未有效的链接。
...
本页的逻辑/ER 建模概念、Mermaid `erDiagram` 语法和 PostgreSQL 18 约束与区间机制分别由后续的官方来源治理锁定。这些来源只支持工具、方法或机制的能力边界，不支持本页的费用申报领域规则，也不证明并发、迁移或性能达标。
