...
export const handleHorizontalArrowKey = (event) => {
  ...
  const nextScrollLeft = region.scrollLeft + direction * 40;
...
- ER 图能表达哪些结构约束，哪些不变量必须留在图外？
- 当前状态、有效期历史、审计记录和事件日志分别回答什么问题？

## 建模目标与输入

[MOD-02 C4 Context 与 Container](/modeling/mod-02)只提供费用申报系统的权威边界与 `银行支付服务` 名称；它不证明本页的数据设计。[MOD-05 概念、逻辑与物理数据模型](/modeling/mod-05)提供 `Employee`、`ExpenseClaim`、`Approval` 和 `PaymentInstruction` 四个起始逻辑实体。本页的六实体模型、字段、基数、半开区间和不变量都是本站原创的教学决定，不是已被证明的生产事实。
...
<div className="diagram-wrapper" role="region" aria-label="费用申报 ER 教学模型，可横向滚动" tabIndex={0} onKeyDown={handleHorizontalArrowKey}>
...
<div className="table-wrapper table-wrapper--mapping" role="region" aria-label="ER 约束覆盖边界表，可横向滚动" tabIndex={0} onKeyDown={handleHorizontalArrowKey}>
...
当前状态回答“现在是什么”；有效期关系回答“某个业务时间段内是什么”；审计记录回答“谁在何时修改了记录”；事件日志记录发生过的事件。四者是不同的证据类型，不能互相替代。本页不是双时态数据库设计，也不采用 Event Sourcing。
...
本页不是完整 DDL，不是可部署 schema，不包含完整双时态数据库设计，也不是 Event Sourcing 实现。缺少生产基数、并发冲突、查询分布、保留、迁移与性能证据时，应停在“待验证”而不是宣称生产就绪。

## 常见失败

常见身份失败是把显示名称、员工号或组织代码当作永不变的实体身份，或者以为有了技术 ID 就已证明业务唯一性。常见关系失败是只写“一对多”却忽略零、一等下界，或把带属性和历史的关系藏在普通连线中。

常见时间失败是根据员工的当前组织归属重算旧申报，或把审计字段、事件日志与 Temporal Event History 当作业务关系历史。常见证据失败是认为 ER 图已经证明时间不重叠、状态前置条件或并发安全，甚至把逻辑 ER 图直接称为生产 schema。

## 与其他模型的衔接

[建模目录](/modeling)提供各类模型入口；[MOD-01 建模总览](/modeling/mod-01)负责按评审问题选择模型；[MOD-05 概念、逻辑与物理数据模型](/modeling/mod-05)为本页提供四个核心逻辑实体；[PR-13 Persistence Ignorance](/principles/pr-13)帮助区分领域含义与持久化机制。[Temporal Saga 与持久执行案例](/cases/temporal-saga-durable-execution)中的 Event History 是控制面恢复记录，不是费用申报的业务关系历史。MOD-08 尚未发布，将来可用状态机补充批准与付款前置条件；本页不创建尚未有效的链接。
...
DBMS 无关的逻辑实体、标识、关系和约束参考 IBM® InfoSphere® Data Architect 软件的官方文档 [Logical Data Models — IBM InfoSphere Data Architect 9.1.1](https://www.ibm.com/docs/en/ida/9.1.1?topic=modeling-logical-data-models)。[Mermaid 11.16.0 Entity Relationship Diagrams](https://github.com/mermaid-js/mermaid/blob/mermaid%4011.16.0/packages/mermaid/src/docs/syntax/entityRelationshipDiagram.md)锁定本页使用的 `erDiagram` 实体、属性、关系标记、标签、基数与可选性语法；Mermaid 11.16.0 语法是渲染工具能力，不作为 ER 理论标准。[PostgreSQL 18 Constraints](https://www.postgresql.org/docs/18/ddl-constraints.html)只支持约束机制行为，[PostgreSQL 18 Range Types](https://www.postgresql.org/docs/18/rangetypes.html)只支持区间边界与排斥约束机制行为。
