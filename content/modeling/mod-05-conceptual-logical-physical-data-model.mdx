...
输入包括业务词汇、规则、样例和冲突，以及身份、生命周期、时间与金额语义。还要记录数据保留、审计、权限与迁移约束，目标 DBMS 及版本，以及可验证的查询、写入和完整性要求；查询分布、关系基数、唯一性、不变量、容量事实和写入竞争若尚未获得证据，就继续标为未知项。先区分来源事实、本站原创的教学假设与未知项，再从业务概念逐层细化，避免以现有数据库倒推业务含义。
...
第四种是把逻辑模型画成流程，让实体连线暗示审批或付款的运行顺序。第五种是从 C4 或 arc42 的系统边界、构建块或文档槽位直接推断生产 schema。第六种是忽略金额、时间、身份、历史和迁移语义，使字段看似齐全却无法解释保留、审计、回填或演进。
...
[MOD-06 ER 模型与关系边界](/modeling/mod-06)进一步深化实体身份、关系基数、图外约束和带有效期的关系历史。

[MOD-09 EventStorming 工作坊建模](/modeling/mod-09)沿用本页的费用申报数据场景，把事件来源、热点、未知项与候选边界假设带回协作工作坊；工作坊排列不替代本页的数据身份、约束或物理实现证据。

[MOD-11 DDD Context Map 建模](/modeling/mod-11)：实体、关系与权威记录可以为 Context 边界提供证据，但数据模型不能单独决定 Bounded Context。
