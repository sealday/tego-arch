...
tags:
  ...
  - 物理数据模型
  - PostgreSQL
summary: 用同一费用申报问题展示业务概念如何逐层加入身份、关系、约束、关系模式和目标数据库物理实现决定。
topic_id: MOD-05
priority: P0
depends_on:
  - MOD-01
...
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
[MOD-02 上下文/容器](/modeling/mod-02)只为本文提供费用申报系统的边界和词汇：员工提交申报，系统组织审批，并向银行支付服务发起付款。MOD-02 的图仍是系统边界与“银行支付服务”名称的权威；本文出现的实体、字段、键、约束、类型选择、关系模式和索引均是本站原创的教学假设，不是 MOD-02 已经证明的事实，也不是某个真实组织的生产设计。

输入包括业务词汇、规则、样例和冲突，以及身份、生命周期、时间与金额语义。还要记录数据保留、审计、权限与迁移约束，目标数据库管理系统及版本，以及可验证的查询、写入和完整性要求；查询分布、关系基数、唯一性、不变量、容量事实和写入竞争若尚未获得证据，就继续标为未知项。先区分来源事实、本站原创的教学假设与未知项，再从业务概念逐层细化，避免以现有数据库倒推业务含义。
...
1. 先识别员工、费用申报、审批和付款四类业务概念，不引入表、键或数据库管理系统。
2. 为逻辑实体加入稳定身份、属性、关系基数、金额与币种语义、唯一性和业务约束，但不选择数据库管理系统。
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
...
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
PostgreSQL 18 物理实现切片讨论 PK、FK、UNIQUE、CHECK、NOT NULL、类型选择和索引候选，不给出完整数据定义语言。其中，费用申报约束、类型选择和索引候选均是本站原创的教学决定；PostgreSQL 18 官方文档只支持相应约束与索引机制行为，不支持费用申报领域规则。稳定标识可选择适合边界的标量类型；金额使用避免二进制浮点误差且明确精度的数值类型，是本站教学决定和待验证要求，不是约束或索引文档对本领域的结论；状态和币种是否使用文本、受约束域或引用表，也需要随演进和治理要求决定。面向员工与状态的申报检索、待处理审批和付款幂等查找可以提出索引候选，但索引候选不证明性能：仍需查询分布、数据基数、写入成本和测量结果。

```mermaid
flowchart LR
  C["业务概念<br/>员工 · 费用申报 · 审批 · 付款"] -->|映射业务词义| L["逻辑实体<br/>员工 · 费用申报 · 审批 · 付款指令"]
  L -->|映射为关系结构| R["可移植关系表<br/>员工表 · 费用申报表 · 审批表 · 付款指令表"]
  R -->|加入目标数据库决定| P["目标数据库物理决定<br/>主键 / 外键 / 唯一 / 检查 / 非空约束<br/>类型选择 · 索引候选"]
...
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
四层使用相同业务词汇且每项新增决定能回到上一层；概念层没有表和键，逻辑层没有数据库管理系统选择；关系模式明确标注为可移植映射；PostgreSQL 18 切片覆盖 PK、FK、UNIQUE、CHECK、NOT NULL、类型选择和索引候选，同时明确它不是生产模式，也不是可部署模式。
...
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
第一种失败是把概念名词直接写成表名，再把表结构称为概念模型。第二种是把带 PK/FK 的通用关系模式称为“完整物理模型”，隐藏数据库管理系统类型、约束语义、访问路径、存储和部署决定。第三种是看到索引候选就宣称查询性能达标，没有核对查询分布、基数、写放大和测量结果。

{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
第四种是把逻辑模型画成流程，让实体连线暗示审批或付款的运行顺序。第五种是从 C4 或 arc42 架构文档模板的系统边界、构建块或文档槽位直接推断生产模式。第六种是忽略金额、时间、身份、历史和迁移语义，使字段看似齐全却无法解释保留、审计、回填或演进。

{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
不要把 PostgreSQL 示例扩张为完整生产数据库：本文没有给出容量、分区、迁移顺序、回滚、权限、备份恢复或性能基线。它不是生产模式，也不是可部署模式。
...
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
[MOD-04 arc42 文档骨架](/modeling/mod-04)可以把本页的层次、假设与验证缺口纳入架构证据；[持久化无知（Persistence Ignorance）](/principles/pr-13)帮助区分领域含义与关系、PostgreSQL 实现决定。[Temporal Saga 与持久执行案例](/cases/temporal-saga-durable-execution)中的 Temporal 事件历史记录是控制面恢复记录，不是业务账本，也不是本文的关系模式；业务身份和关系约束仍需单独建模。

{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
[MOD-06 实体关系模型与关系边界](/modeling/mod-06)进一步深化实体身份、关系基数、图外约束和带有效期的关系历史。

{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
[MOD-09 事件风暴工作坊建模](/modeling/mod-09)沿用本页的费用申报数据场景，把事件来源、热点、未知项与候选边界假设带回协作工作坊；工作坊排列不替代本页的数据身份、约束或物理实现证据。

{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
[MOD-11 领域驱动设计上下文映射（DDD Context Map）](/modeling/mod-11)：实体、关系与权威记录可以为上下文边界提供证据，但数据模型不能单独决定限界上下文。
...
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
逻辑层为 Employee、ExpenseClaim、Approval 和 PaymentInstruction 指定稳定身份。ExpenseClaim 关联一名 Employee，并携带金额、币种、发生日期和状态；Approval 关联一份 ExpenseClaim，并记录审批者、结论和决定时间；PaymentInstruction 关联已获准付款的申报，并用唯一业务键避免重复发起。约束包括金额必须大于零、币种必须明确、同一审批步骤不得重复决定，以及付款只针对满足批准条件的申报。这里仍不选择数据库管理系统。
