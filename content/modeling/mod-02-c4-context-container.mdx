...
Context 图回答“系统与谁协作”，Container 图回答“系统内部有哪些可运行或存储单元”。它们用于沟通静态边界，不是运行时序列或部署证明。[建模总览](/modeling/mod-01)先按评审问题选择模型，[建模目录](/modeling)提供其他模型入口。
...
下面是本站原创的 Context→Container 两层映射。先看上层如何把目标系统与员工、银行支付服务分开，再沿“展开”关系检查下层的可运行与存储单元；箭头只表达关系，不声称调用顺序。
...
Context 层保留系统整体，Container 层只展开目标系统；银行支付服务仍在边界外。图中关系标签与连线分离，便于辨认方向，但这些箭头仍不增加运行顺序、部署位置或故障切换方面的证据。
...
[Component、Dynamic 与 Deployment](/modeling/mod-03)继续使用本页的费用申报系统元素名，分别补申报 API 内部责任、关键场景时间顺序与生产实例落点。[架构风格比较框架](/styles/sty-00)帮助解释容器为何采用某种连接器与数据边界；ADR 记录为什么选择这些边界。模型之间应共享名称，但不互相冒充。

## 完整演练

输入：费用申报系统供员工提交费用，审批后调用银行支付服务；团队已知有 Web 应用、申报 API、申报数据库和支付任务执行器。
