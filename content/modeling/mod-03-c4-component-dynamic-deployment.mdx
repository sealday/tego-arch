...
输入包括 MOD-02 已审计的系统与 Container 边界，以及每个视图各自的一条评审问题和证据。Component 需要代码、依赖与变化所有权证据，缺失项必须标为显式假设；Dynamic 需要单一用例、可执行测试与追踪证据；Deployment 需要环境清单、配置与观测，并记录事实截止时间为 2026-07-30。没有明确评审问题或证据边界就不增加视图。

这三个视图必须沿用员工、Web 应用、申报 API、申报数据库、支付任务执行器与银行支付服务等固定名称。视图之间共享词汇，不共享证明范围。本文费用申报系统拓扑是教学演练假设，并非经过生产盘点确认的生产拓扑事实。
...
4. 命名一个教学演练中的生产环境，分别列出部署节点与 Container 实例，再核对每个实例来自哪个 MOD-02 Container；数据库部署节点承载申报数据库 Container 实例。
...
交互顺序仅表达这一次场景的相对先后，不承诺线程、消息协议或耗时。Dynamic 不等于性能报告或生产追踪，也不证明真实调用一定按图发生；性能、追踪与失败证据仍来自运行系统。
...
视图只命名教学演练中的“生产环境”：`员工终端`、`Web 节点`、`API 节点`、`数据库节点` 与 `任务执行节点` 都是部署节点；其中的 `Web 应用实例`、`申报 API 实例`、圆柱形 `申报数据库实例` 与 `支付任务执行器实例` 都是 Container 实例，数据库部署节点承载申报数据库 Container 实例。`基础设施节点` 是 Deployment 可选类型，本最小教学图未引入对应实例，也不额外添加 DNS 或 LB。`银行支付服务` 仍在系统边界之外。不要把演练映射误写成已经盘点确认的生产事实。

Deployment 不证明容量或故障切换能力，也不能仅凭多实例图形声称高可用；容量、故障切换和恢复能力需要配置、测试与生产观测支持。

## 完成判断

Component 只展开申报 API，四个责任单元都能用“负责什么、向谁提供什么接口、谁拥有变化”回答；不需要这一层时明确删除。Dynamic 只有一个已审批费用付款场景，参与者名称与 MOD-02 一致。Deployment 明确写出教学演练假设，区分部署节点与 Container 实例，并说明基础设施节点只是未实例化的可选类型。每个视图都有可复述的“不证明”声明。
...
[MOD-07 UML 选图指南](/modeling/mod-07)从评审问题比较 sequence 与 deployment 等 UML 视图；本页则继续限定 C4 Dynamic 与 Deployment 的具体观察尺度和证据边界。
...
[arc42 文档骨架](/modeling/mod-04)把本页的静态、运行与部署证据放回可裁剪的整体文档结构，同时保留各视图“不证明什么”的边界。

## 完整演练

输入是已审计的费用申报系统教学案例：员工通过 Web 应用提交费用，申报 API 持有业务规则与数据，支付任务执行器调用银行支付服务；评审问题分别是业务变化归属、已审批费用的付款协作、生产实例落点。拓扑细节仍是截至 2026-07-30 的教学演练假设，不代表真实生产环境。

先判断 Component 是否必要。团队正要拆分审批与付款变更责任，因此只展开申报 API，得到提交用例、审批策略、付款编排、持久化端口四个责任单元。接着只画已审批费用付款：员工提交，提交用例验证审批状态并创建任务，执行器调用银行。最后声明一个教学演练中的生产环境，区分承载节点与 Web/API/数据库/执行器 Container 实例，其中数据库部署节点承载申报数据库 Container 实例。评审结束时，团队同时记录代码一致性、性能追踪、容量与故障切换都没有被这些图证明。
