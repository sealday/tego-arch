...
视图只命名教学演练中的“生产环境”：`员工终端`、`Web 节点`、`API 节点`、`数据库节点` 与 `任务执行节点` 都是部署节点；其中的 `Web 应用实例`、`申报 API 实例`、圆柱形 `申报数据库实例` 与 `支付任务执行器实例` 都是 Container 实例。`银行支付服务` 仍在系统边界之外。不要把演练映射误写成已经盘点确认的生产事实。
