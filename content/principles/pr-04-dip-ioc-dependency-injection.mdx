...
tags:
  ...
  - 控制反转
  - 依赖注入
summary: 分开源代码依赖、运行时控制和对象装配三条边，避免把接口或依赖注入容器误当成依赖倒置。
topic_id: PR-04
priority: P0
depends_on:
  - PR-01
...
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
**来源事实：** Robert C. Martin 的 DIP 论述要求高层政策不依赖低层细节，二者依赖抽象；Fowler 的文章区分 IoC 容器、依赖注入与服务定位器。**推断：** 评审必须分别画出编译时依赖、运行时调用和对象创建，不能看到构造器注入就推定业务政策已经摆脱基础设施细节。

{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
要保护的是策略独立性：核心规则可以在不理解数据库软件开发工具包（Software Development Kit，SDK）、网络客户端或框架生命周期的情况下表达和验证。IoC 与 DI 可以帮助实现这一点，却也可能只改变控制和装配而没有改变依赖方向。
...
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
[信息隐藏与封装](/principles/pr-01)决定哪些低层决策不应泄漏给策略；[组合优于继承](/principles/pr-05)说明 DI 常用的对象组合为何能让策略独立替换；[快速失败（Fail Fast）、故障安全（Fail Safe）与优雅降级（Graceful Degradation）](/principles/pr-07)把故障策略放在拥有正确控制权的边界；[为演化设计](/principles/pr-08)利用依赖方向在迁移期间替换政策与实现；[最小权限、安全默认值与纵深防御](/principles/pr-09)把政策独立性应用到运行时授权，[CQS、命令查询职责分离（Command Query Responsibility Segregation，CQRS）与读写分离](/principles/pr-11)则把命令与查询责任的分离同注入机制区分开；[开闭原则（Open/Closed Principle）与接口隔离原则（Interface Segregation Principle）](/principles/pr-12)提醒依赖方向本身并不能证明应当建立扩展面；[领域模型与存储解耦](/principles/pr-13)决定持久化映射与领域规则如何放置；[GRASP 责任分配](/principles/pr-14)决定某项协作责任由哪个对象承担。DIP 只约束依赖方向，不会自动决定持久化位置或责任选择；组合承担运行时组装，其他原则再约束所有权、失败和变化，不能互相替代。
...
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
{/* terminology-exempt: unknown-english-term | reason: 引用原题、产品专名、主题标识或固定内容合同按原样保留 */}
选择 B 的依据不是“方便模拟对象”，而是账单政策和数据库供应商具有不同变化原因。应用启动器控制对象创建（DI），Web 框架控制何时调用处理器（IoC），数据库适配器依赖政策抽象（DIP）。若工具只是一次性导入固定数据库，方案 A 可能更简单。[OpenAI Agents SDK 案例](/cases/openai-agents-sdk)可继续检查框架回调、扩展点和应用政策之间的控制边界。
