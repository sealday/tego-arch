...
先看本站原创插图中的三层检查与右侧版本流：source、wire、semantics 分别验证旧端与新端，互操作边界另查协议和行为；迁移失败则进入 breaking-change 分支。

![分层检查源码、传输与语义兼容以及版本迁移边界](/img/illustrations/qa-07-compatibility-version-migration.png)

*图：接口形状相同不等于语义兼容；移除旧版本前还要验证互操作行为与迁移证据。*

插图不把共享 OAS、transport 或 version label 当成语义兼容保证。迁移按以下本站原创状态流推进，任何阶段失败都回到可兼容状态：
...
[QA-04 可扩展性](/quality-attributes/qa-04)涉及新旧分区路由与规模单位，[QA-05 Security/privacy](/quality-attributes/qa-05)约束身份、授权和数据目的在版本迁移中的连续性，[QA-06 可维护性](/quality-attributes/qa-06)约束合同变化的 blast radius 和验证。
