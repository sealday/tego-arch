...
| 度量结果 | 影响范围、动作审批、恢复时间、用户成功和后续负责人均可核对 |
...
[安全性（Security）与隐私](/quality-attributes/qa-05)约束遥测数据和运行权限，[安全保障（Safety）](/quality-attributes/qa-09)要求物理风险动作具有更严格的安全边界。
...
- [Google SRE Book — Monitoring Distributed Systems](https://sre.google/sre-book/monitoring-distributed-systems/)：用于监控与告警的事实摘要；不作为通用阈值或当前供应商政策。
